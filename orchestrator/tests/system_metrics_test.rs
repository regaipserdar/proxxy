...
async fn test_system_metrics_database_storage() {
    ...
    db.upsert_agent("test-agent", "Test Agent", "localhost", "1.0.0")
        .await
        .unwrap();
    ...
    // Retrieve metrics
    let retrieved = db
        .get_recent_system_metrics(Some("test-agent"), 1)
        .await
        .unwrap();
...
async fn test_system_metrics_database_query_filtering() {
    ...
    let agent1_metrics = db
        .get_recent_system_metrics(Some("agent-1"), 10)
        .await
        .unwrap();
...
async fn test_system_metrics_data_consistency_over_time() {
    ...
    // First, create the agent record to satisfy foreign key constraint
    db.upsert_agent(agent_id, "Consistency Test Agent", "localhost", "1.0.0")
        .await
        .unwrap();
    ...
    let metrics = db
        .get_recent_system_metrics(Some(agent_id), 20)
        .await
        .unwrap();
    ...
    let latest_network = latest_metrics
        .metrics
        .as_ref()
        .unwrap()
        .network
        .as_ref()
        .unwrap();
    let oldest_network = oldest_metrics
        .metrics
        .as_ref()
        .unwrap()
        .network
